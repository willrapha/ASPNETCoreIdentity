...
namespace IdentityWithDapper.Data
{
        ...
        IUserRoleStore<ApplicationUser>
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested(); // No caso da operação ter precisado ser cancelada

            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync(cancellationToken); // A qualquer momento a tarefa pode ser cancelada quando solicitado
                ...
                user.Id = await connection.QuerySingleAsync<int>(query, user); // Executamos a query
            }
        ...
        {
            // Nao utilizamos o cancellationToken porque nao fazemos acesso ao banco de dados
        ...
        {
            user.Email = email;
            return Task.FromResult(0); // 0 - devido nao ser esse medo o responsavel por atualizar o email
        }

        public Task<string> GetEmailAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            return Task.FromResult(user.Email); // Retornamos o Email
        }
        ...
        public async Task<ApplicationUser> FindByEmailAsync(string normalizedEmail, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                // Padrao - query , objetos que estamos passando no caso o 'new { normalizedEmail }' que esta vindo como parametro
            }
        }
        ...
        public async Task<bool> IsInRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            {
                var roleId = await connection.ExecuteScalarAsync<int?>("SELECT [Id] FROM [ApplicationRole] WHERE [NormalizedName] = @normalizedName", new { normalizedName = roleName.ToUpper() });
                if (roleId == default(int)) return false; // default(int) é 0
